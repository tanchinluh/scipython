...
int ScilabPythonEnvironmentWrapper::wrapFloat(double * x, const bool isRef) const
{
    if (helper.getWrapSingleWithNumpy())
    {
        return wrapData<double, float, npy_float>(x, 1, isRef);
    }
    else
    {
        return wrapData<double, float>(x, isRef);
    }
}
    
int ScilabPythonEnvironmentWrapper::wrapFloat(double * x, int xSize, const bool isRef) const
{
    return wrapData<double, float, npy_float>(x, xSize, isRef);
}

int ScilabPythonEnvironmentWrapper::wrapFloat(double * x, int xSize, int xSizeCol, const bool isRef) const
{
    return wrapData<double, float, npy_float>(x, xSize, xSizeCol, isRef);
}
...
int ScilabPythonEnvironmentWrapper::wrapList(int len, const int * const ids) const
{
    if (len <= 0)
    {
	PyObject * l = PyList_New(0);
	return scope.addObject(l);
    }

    PyObject * l = PyList_New(len);
    for (int i = 0; i < len; ++i)
    {
	PyObject * obj = scope.getObject(ids[i]);
	if (!obj)
	{
	    throw ScilabPythonException(__LINE__, __FILE__, gettext("Invalid object with id %d"), ids[i]);
	}
	Py_INCREF(obj);
...
int ScilabPythonEnvironmentWrapper::wrapPoly(int len, const double * const coefs) const
{
