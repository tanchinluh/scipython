...
int ScilabPythonEnvironment::extract(int id, int * args, int argsSize)
{
	...
	Py_INCREF(value);
        ...
        writeLog("extract", "(from dictionary) returned id: %d.", ret);
    ...
    else if (PyTuple_Check(obj))
    {
        if (argsSize != 1)
        {
            throw ScilabPythonException(__LINE__, __FILE__, gettext("Cannot extract more than one element from a tuple"));
        }

	PyObject * entry = scope.getObject(*args);
	Py_ssize_t index;
	if (PyFloat_Check(entry))
	{
	    index = (Py_ssize_t)PyFloat_AS_DOUBLE(entry);
	}
	else if (PyInt_Check(entry))
	{
	    index = PyInt_AsSsize_t(entry);
	}
	...
	{
	    value = PyTuple_GetItem(obj, index);
	}
	else if (index < 0 && size + index >= 0)
	{
	    value = PyTuple_GetItem(obj, size + index);
	}
	else
	{
            throw ScilabPythonException(__LINE__, __FILE__, gettext("Invalid index in tuple"));
	}
	Py_INCREF(value);
	
	int ret = scope.addObject(value);
	writeLog("extract", "(from tuple) returned id: %d.", ret);
    ...
    else if (PyList_Check(obj))
    {
        if (argsSize != 1)
        {
            throw ScilabPythonException(__LINE__, __FILE__, gettext("Cannot extract more than one element from a list"));
        }
	...
	if (index >= 0 && index < size) 
	{
	    value = PyList_GetItem(obj, index);
	}
	else if (index < 0 && size + index >= 0)
	{
	    value = PyList_GetItem(obj, size + index);
	}
	else
	{
            throw ScilabPythonException(__LINE__, __FILE__, gettext("Invalid index in list"));
	}
	Py_INCREF(value);
	
	int ret = scope.addObject(value);
	writeLog("extract", "(from list) returned id: %d.", ret);
...
std::string ScilabPythonEnvironment::getrepresentation(int id)
{
    ...
    PyObject * repr = PyObject_Repr(obj);
    std::string str(PyString_AsString(repr));
    Py_DECREF(repr);
...
int * ScilabPythonEnvironment::invoke(int id, const char * methodName, int * args, int argsSize)
{
    ...
    std::string _mname(methodName);
    std::vector<std::string> pathElements;
    std::vector<PyObject *> pathObjects;
    std::size_t prev(0);
    std::size_t pos = _mname.find_first_of(".");
    while (pos != std::string::npos)
    {
	pathElements.push_back(_mname.substr(prev, pos - prev));
	prev = pos + 1;
	pos = _mname.find_first_of(".", prev);
    }
    pathElements.push_back(_mname.substr(prev));
    
    if (!helper.getShowPrivate() && pathElements.back().at(0) == '_')
    {
    ...
    PyObject * method = obj;
    for (std::vector<std::string>::iterator i = pathElements.begin(); i < pathElements.end(); ++i)
    {
	if (!PyObject_HasAttrString(method, i->c_str()))
	{
	    for (std::vector<PyObject *>::iterator j = pathObjects.begin(); j < pathObjects.end(); ++j)
	    {
		Py_DECREF(*j);
	    }
	    throw ScilabPythonException(__LINE__, __FILE__, gettext("Invalid attribute name: %s"), i->c_str());
	}
	
	method = PyObject_GetAttrString(method, i->c_str());
	if (!method)
	{
	    for (std::vector<PyObject *>::iterator j = pathObjects.begin(); j < pathObjects.end(); ++j)
	    {
    ...
    PyObject * pArgs;
    if (argsSize >= 1 && *args == VOID_OBJECT)
    {
	pArgs = PyTuple_New(0);
    }
    else
    {
	pArgs = PyTuple_New(argsSize);
    ...
    if (!result)
    {
        if (PyErr_Occurred())
        {
	    if (argsSize && PyDict_Check(scope.getObject(args[argsSize - 1])))
	    {
		PyErr_Clear();
		pArgs = PyTuple_New(argsSize - 1);
		for (int i = 0; i < argsSize - 1; ++i)
		{
		    PyObject * obj = scope.getObject(args[i]);
		    Py_INCREF(obj);
		...
		result = PyObject_Call(method, pArgs, scope.getObject(args[argsSize - 1]));
		Py_DECREF(pArgs);
		if (!result)
		{
		    if (PyErr_Occurred())
...
int ScilabPythonEnvironment::getfieldtype(int id, const char * fieldName)
{
    ...
    if (!PyObject_HasAttrString(obj, fieldName))
    {
        writeLog("getfieldtype", "Return NONE (no such field %s).", fieldName);
        return -1;
    }
    ...
    if (!field)
    {
        writeLog("getfieldtype", "Return NONE (field exists but NULL).");
        return -1;
    }
...
void ScilabPythonEnvironment::autoremoveobject(int id)
{
    scope.removeObject(id);
}
...
void ScilabPythonEnvironment::writeLog(const std::string & fun, const std::string str, ...) const
{
    if (traceEnabled)
    {
