...
int PythonVariablesScope::addObject(PyObject * obj) const
{
    ...
    ScilabAutoCleaner::registerVariable(env.getEnvId(), ret);
...
PyObject * PythonVariablesScope::removeObjectWithoutDecref(int pos) const
{
	...
	PyObject * repr = PyObject_Repr(obj);
        ...
        if (strlen(_repr) > MAX_STR_LENGTH)
        {
            env.writeLog("addObject", "Remove object with id %d, repr is %." MAX_STR_LENGTH_STR "s...(truncated) and refcnt=%d (before DECREF).", pos + 1, _repr, obj->ob_refcnt);
        }
        else
        {
            env.writeLog("addObject", "Remove object with id %d, repr is %s and refcnt=%d (before DECREF).", pos + 1, _repr, obj->ob_refcnt);
        }
